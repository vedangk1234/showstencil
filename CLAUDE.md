...
* Auto-detection runs in /api/sync, not in a separate cron (Day 18): Detection is triggered once during the user's first sync when `existingAutoCount === 0`. Running it in the cron would require every cron iteration to re-check every user; running it in sync means it fires naturally on the next sync after onboarding and never again. The check is O(1) — one COUNT query per sync for users who already have competitors.
...
* Auto-detection never blocks the sync response (Day 18): `detectAndAssignCompetitors` is called with `await` inside a try/catch. The catch logs the error and continues — the sync JSON response is built and returned regardless of detection outcome. If detection fails mid-run (e.g. quota exhausted after Tier 1), whatever was already inserted is kept and the next sync attempt will find `existingAutoCount > 0` and skip re-running detection. This means partial detection is permanent — the missing tier will not auto-fill unless the competitors are manually deleted.
* Auto-detected competitors are permanent after assignment (Day 18): The `existingAutoCount === 0` condition ensures detection runs exactly once per user lifetime. Subsequent syncs skip the entire block in milliseconds. Competitors assigned by auto-detection are never replaced by the system — only the user can deactivate them (`is_active = false`) or they can be replaced via the manual search slot.
