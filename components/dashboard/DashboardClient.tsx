...
export default function DashboardClient({
  ...
  const earlier = snapshots[Math.max(0, snapshots.length - 8)]
  ...
  const gapRows = gapScore ? [
    { label: 'Avg views / video', score: gapScore.views_gap_score },
    { label: 'Click-through rate', score: gapScore.ctr_gap_score },
    { label: 'Watch time',         score: gapScore.watch_time_gap_score },
    { label: 'Upload frequency',   score: gapScore.upload_frequency_gap_score },
    { label: 'Topic coverage',     score: gapScore.topic_coverage_gap_score },
  ] : []
  ...
  return (
    <Shell>
      ...
      <div style={{
          ...
          <p style={{ fontSize: 13.5, color: T.ink2, margin: '8px 0 0', maxWidth: '56ch' }}>
            {gapScore ? (
              <>
                ...
                {gapScore.estimated_revenue_gap > 0 && (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end' }}>
          <Pill>
            ...
            <strong style={{ color: T.ink, marginLeft: 4 }}>{competitors.length}</strong> channels
          </Pill>
          <Pill>
      ...
      <div style={{
        ...
        <Col title="Competitors" sub={`${competitors.length} tracked`} actions={['All', 'Tier 1', 'Tier 2', 'Dominators']}>
          {competitorsSorted.length === 0 ? (
            ...
            const compAvg = c.total_views && c.subscriber_count
              ? Math.round(c.total_views / Math.max(1, c.subscriber_count) * 1000) / 10
              : null
            const mult = compAvg && userAvg ? compAvg / userAvg : null
            return (
              <div key={c.id} style={{
                display: 'grid',
                ...
                <span style={{
                  ...
                  color: mult && mult > 1 ? T.accent : T.red,
                }}>
                  {mult ? `${mult.toFixed(1)}×` : '—'}
                </span>
              </div>
      ...
      <div style={{
        ...
        <Col title="Top ideas" sub="this week" actions={['all']}>
          ...
          {(!latestDigest?.video_ideas || latestDigest.video_ideas.length === 0) && (
      ...
      <div style={{ marginTop: 24, border: `1px solid ${T.line}` }}>
        ...
        {recentDigests.length > 0 ? recentDigests.map(d => {
