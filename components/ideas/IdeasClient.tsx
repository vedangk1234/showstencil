...
function Section({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ marginTop: 16 }}>
      ...
      <p style={{ color: '#999999', fontSize: 13, lineHeight: 1.65, margin: 0 }}>
        {children}
      </p>
    </div>
  )
...
function IdeaCard({
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${isDone ? '#111111' : '#1a1a1a'}`,
      borderRadius: 10,
      padding: '20px 22px',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 4 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          <span style={{
            fontSize: 11,
            fontWeight: 700,
            color: scoreColor(idea.opportunity_score),
            background: scoreBg(idea.opportunity_score),
            border: `1px solid ${scoreBorder(idea.opportunity_score)}`,
            borderRadius: 4,
            padding: '3px 9px',
            whiteSpace: 'nowrap',
          }}>
      ...
      {/* Why now */}
      <p style={{ color: '#666666', fontSize: 12, fontStyle: 'italic', lineHeight: 1.6, margin: '6px 0 0' }}>
        {idea.why_now}
      </p>

      {/* Sections */}
      <Section label="Thumbnail brief">{idea.thumbnail_description}</Section>
      <Section label="Content brief">{idea.content_brief}</Section>
      <Section label="Duration logic">{idea.duration_reasoning}</Section>
      {idea.topic_source && (
        <Section label="Why we suggested this">{idea.topic_source}</Section>
      )}
          ...
          {STAGES.map((label, i) => {
            ...
            return (
              <div key={label} style={{
                ...
                <span style={{
                  fontSize: 12,
                  color: done ? '#4ade80' : active ? '#ffffff' : '#333333',
                }}>
                  {label}
                </span>
              </div>
...
export function IdeasClient({
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 429) {
          // Limit hit — refresh to show cached ideas and correct button state
  ...
  const canRegenerate = !regenerateAvailableAt || new Date(regenerateAvailableAt) <= new Date()
  const nextRefreshLabel = regenerateAvailableAt
    ? `Next refresh on ${formatDate(regenerateAvailableAt)}`
    : null

  // ── Empty state (no ideas at all, not generating) ──────────────────────
          ...
          {activeIdeas.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: 14 }}>
              {activeIdeas.map((idea) => (
                <IdeaCard key={idea.id} idea={idea} onPlan={handlePlan} onMade={handleMade} />
              ))}
            </div>
          )}
          ...
          {doneIdeas.length > 0 && (
            <div style={{ marginTop: 32 }}>
              ...
              {doneExpanded && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: 14 }}>
                  {doneIdeas.map((idea) => (
                    <IdeaCard key={idea.id} idea={idea} onPlan={handlePlan} onMade={handleMade} isDone />
                  ))}
                </div>
              )}
            </div>
